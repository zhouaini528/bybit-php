...
trait SocketGlobal
{
    ...
    {
        $address = isset($this->config['global']) ? explode(':',$this->config['global']) : ['0.0.0.0', '2210'];
    ...
    {
        $address = isset($this->config['global']) ? $this->config['global'] : '0.0.0.0:2210';
    ...
    protected function keysecretClear($keysecret)
    {
        do {
            $old_value = $new_value = $this->client->keysecret;
            unset($new_value[$keysecret['key']]);
        }
        while(!$this->client->cas('keysecret', $old_value, $new_value));
    }
